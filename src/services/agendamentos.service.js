...
const getHorarios = async (dia, idBarbeiro) => {
    ...
    const horAlmoco = ['12:00', '12:30', '13:00', '13:30'];

    const todosIndis = todosHorariosIndisponiveis.concat(horAlmoco);
    ...
    if (diaAtual) {
        ...
        if (todosIndis.length > 0) {
            const horariosDisponiveis = await horariosDispon(hourFinalFormat, intervalo, hourLimit, todosIndis);
            return horariosDisponiveis;
        }
    ...
    if (!diaAtual) {
        ...
        if (todosIndis.length > 0) {
            const horariosDisponiveis = await horariosDiasFuturos(hourFinalFormat, intervalo, hourLimit, todosIndis);
            return horariosDisponiveis;
        }
...
const checkAgendamentoCliente = async (id) => {
        const existAgendamento = await models.agendamentos.findOne({
            where: {
              id_cliente: id,
              status: 'agendado',
            },
        });
        return existAgendamento;
};
  ...
  const getAgendaId = async (id) => {
    const resultAgendamentos = await models.agendamentos.findAll({
      ...
      include: [
        {
          model: models.barbeiros,
          attributes: ['name', 'image'], // traz apenas os atributos 'id' e 'nome' da tabela barbeiros
        },
        {
          model: models.servicos,
          attributes: ['id', 'description', 'price'], // traz apenas os atributos 'id' e 'descricao' da tabela servicos
...
const postAgendamento = async (payload) => {
    ...
    try {
        ...
        return { message: 'Agendamento concluido com sucesso', agenda: resultAgendamento.id};
    } catch (error) {
...
module.exports = { getHorarios, postAgendamento, getHorariosReservados, getAgendamentos, checkAgendamentoCliente, updateStatus, getAgendaId };
